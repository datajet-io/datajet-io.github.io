...
'<span class="dj-item-product"><strong class="dj-item-brand">'+c+'</strong><span class="dj-item-title">'+a.ti+'</span><strong class="dj-item-price">R$'+a.cp+"</strong></span>"):d=a.ti:2==a.ty&&(d=a.br?a.br+' <span class="dj-sep">em</span> '+(a.ti?a.ti:a.ca):a.ti+(a.ca?' <span class="dj-sep">em</span> '+a.ca:""));d='<a class="dj-autocomplete-suggestion'+e+'" data-link="'+a.li+'" href="//'+a.li+'">'+d+"</a>";a.ct&&(d='<div class="dj-autocomplete-category">'+a.ct+"</div>"+d);return d},onSelect:function(a,
e,c){c.getAttribute("data-link")&&(window.location.href=window.location.protocol+"//"+c.getAttribute("data-link"))},source:function(a,e){var c=[],d=[],g=[],f=this,h=f.isMobile()?15:20;Datajet.sae(window.FEED_KEY,h,"search").get(a,function(a){for(var b=0;b<a.length;b++)0==g.length&&(a[b].ct="Search Results"),g.length<(f.isMobile()?5:3)&&g.push(a[b]);e(g.concat(c).concat(d))});Datajet.sae(window.FEED_KEY,h,"category").get(a,function(a){for(var b=0;b<a.length;b++)0==c.length&&(a[b].ct="Category Suggestions"),
c.length<(f.isMobile()?5:3)&&c.push(a[b]);e(g.concat(c).concat(d))});Datajet.sae(window.FEED_KEY,h,"product").get(a,function(a){for(var b=0;b<a.length;b++)0==d.length&&(a[b].ct="Products"),d.length<(f.isMobile()?4:3)&&d.push(a[b]);e(g.concat(c).concat(d))})}})});
